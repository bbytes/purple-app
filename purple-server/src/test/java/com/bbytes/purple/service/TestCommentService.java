...
public class TestCommentService extends PurpleBaseApplicationTests {
	...
	@Test
	public void testCommentCountForStatus() {
		...
		Comment comment1 = new Comment("test comment", testUser, status1);
		commentService.save(comment1);
		
		long count = commentService.getCountByStatus(status1);

		Assert.assertTrue(count > 0);
		
		Comment comment2 = new Comment("test comment 2", testUser, status1);
		commentService.save(comment2);
		
		Comment comment3 = new Comment("test comment 3", testUser, status1);
		commentService.save(comment3);
		
		Comment comment4 = new Comment("test comment 4", testUser, status1);
		commentService.save(comment4);
		
		count = commentService.getCountByStatus(status1);

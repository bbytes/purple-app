 ...
 */
@Data
public class SyncJiraProjectAndUserAndIssueExecutor implements Runnable {

	private static final Logger logger = LoggerFactory.getLogger(SyncJiraProjectAndUserAndIssueExecutor.class);
	...
	public SyncJiraProjectAndUserAndIssueExecutor(User loggedInUser, IntegrationService integrationService,
			JiraIntegrationService jiraIntegrationService, Map<String, Object> emailBody, List<String> emailList,
			NotificationService notificationService, SpringProfileService springProfileService) {
	...
	public void run() {
		...
		try {
			...
			jiraIntegrationService.syncJiraProjectsAndUserAndIssues(integration, loggedInUser);
